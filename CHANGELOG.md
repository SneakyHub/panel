...
* Multiple XSS vulnerabilities in the admin area ([GHSA-384w-wffr-x63q](https://github.com/sneakypanel/panel/security/advisories/GHSA-384w-wffr-x63q))
...
* Added a telemetry service to collect anonymous metrics from the panel, this feature is *enabled* by default and can be toggled using the `sneakypanel_TELEMETRY_ENABLED` environment variable.
...
* Added the `server:settings.description` activity log event for when a server description is changed.
* Added the ability to cancel file uploads in the file manager for a server.
* Added a telemetry service to collect anonymous metrics from the panel, this feature is disabled by default and can be toggled using the `sneakypanel_TELEMETRY_ENABLED` environment variable.
...
* API keys generated by the system now set a prefix to identify them as sneakypanel API keys, and if they are client or application keys. This prefix looks like `ptlc_` for client keys, and `ptla_` for application keys. Existing API keys are unaffected by this change.
...
* **DEPRECATED**: The use of `sneakypanel\Models\AuditLog` is deprecated and all references to this model have been removed from the codebase. In the next major release this model and table will be fully dropped.
...
## v1.6.6
### Fixed
* **[security]** Fixes a CSRF vulnerability for both the administrative test email endpoint and node auto-deployment token generation endpoint. [GHSA-wwgq-9jhf-qgw6](https://github.com/sneakypanel/panel/security/advisories/GHSA-wwgq-9jhf-qgw6)
...
* Updates all of the default eggs shipped with the Panel to reference new [`ghcr.io` yolks repository](https://github.com/sneakypanel/yolks).
...
* Adds automated self-upgrade process for sneakypanel Panel once this version is installed on servers. This allows users to update by using a single command.
...
## v1.0.0
sneakypanel 1.0 represents the culmination of over two years of work, almost 2,000 commits, endless bug and feature requests, and a dream that
...
* **[Security]** Addresses a Client API vulnerability that would allow a user to list all servers on the system ([`GHSA-6888-7f3w-92jx`](https://github.com/sneakypanel/panel/security/advisories/GHSA-6888-7f3w-92jx))
...
* Fixed `sneakypanel.environment_variables` to be used correctly for global environment variables. The wrong config variable name was being using previously.
...
* Server short-codes are now based on the UUID as they were in previous versions of sneakypanel.
...
### Changed
* Renamed session cookies from `laravel_session` to `sneakypanel_session`.
...
* Emails sending with 'sneakypanel Panel' as the from name. Now configurable by using `php artisan sneakypanel:mail` to update.
...
* Many structural changes to the database and `sneakypanel\Models` classes that would flood this changelog if they were all included. All required migrations included to handle database changes.
...
* Attempting to reset a password for an account that does not exist no longer returns an error, rather it displays a success message. Failed resets trigger a `sneakypanel\Events\Auth\FailedPasswordReset` event that can be caught if needed to perform other actions.
...
* Ability to create a node through the CLI using `sneakypanel:node` as well as locations via `sneakypanel:location`.
...
### Added
* New API route to return allocations given a server ID. This adds support for a community-driven WHMCS module :rocket: available [here](https://github.com/hammerdawn/sneakypanel-WHMCS).
...
* Panel now displays `sneakypanel Panel` as the company name if one is not set.
...
* Added support for file copying through the file manager. [#127](https://github.com/sneakypanel/Panel/issues/127)
...
* Passing `?daemon=true` flag to [`/api/servers/:id`](https://sneakypanel.readme.io/v0.5.0/reference#single-server) will return the daemon stats as well as the `daemon_token` if using HTTPS.
...
* **ALL** API routes previously returning paginated result sets, or result sets nested inside a descriptive block (e.g. `servers:`) have been changed to return a single array of all associated items. Please see the [updated documentation](https://sneakypanel.readme.io/v0.5.0/reference) for how this change might effect your API use.
* API route for [`/api/users/:id`](https://sneakypanel.readme.io/v0.5.0/reference#single-user) now includes an array of all servers the user is set as the owner of.
...
* Team Fortress named 'Insurgency' in panel in database seeder. ([#96](https://github.com/sneakypanel/Panel/issues/96), PR by [@MeltedLux](https://github.com/MeltedLux))
...
* Fixes display where an extra space was added to the end of SFTP passwords when they were copied from the panel. [#116](https://github.com/sneakypanel/Panel/issues/116), thanks [@OrangeJuiced](https://github.com/OrangeJuiced)
...
### Fixed
* File manager would do multiple up-down-up-down loading actions if you escaped renaming a file. Fixed the binding issue. [#122](https://github.com/sneakypanel/Panel/issues/122)
* File manager actions would not trigger properly if text in a row was used to right-click from.
* File manager rename field would not disappear when pressing the escape key in chrome. [#121](https://github.com/sneakypanel/Panel/issues/121)
...
### Fixed
* Wrong error highlighting of overallocate fields on Node creation ([#90](https://github.com/sneakypanel/Panel/issues/90), thanks [@schrej](https://github.com/schrej))
* Server link in navbar directed to 404 link (PR by [@Randomfish132](https://github.com/Randomfish132))
* Composer fails to finish ([#92](https://github.com/sneakypanel/Panel/issues/92), PR by [@schrej](https://github.com/schrej), thanks [@parkervcp](https://github.com/parkervcp))
...
* Adds support for viewing SFTP password within the panel ([#74](https://github.com/sneakypanel/Panel/issues/74), thanks [@ET-Bent](https://github.com/ET-Bent))
...
### Fixed
* Fixes password auto-generation on 'Manage Server' page. ([#67](https://github.com/sneakypanel/Panel/issues/67), thanks [@ET-Bent](https://github.com/ET-Bent))
...
* Fixes the server name in the header not linking to the server correctly. ([#79](https://github.com/sneakypanel/Panel/issues/79), thanks [@xX1bumblebee1Xx](https://github.com/xX1bumblebee1Xx))
* Fixes bug where non-admins could not see command box on servers. ([#83](https://github.com/sneakypanel/Panel/issues/83), thanks [@xX1bumblebee1Xx](https://github.com/xX1bumblebee1Xx))
* Fixes bug where files could not be uploaded through the "click and select" system, only through "drag and drop." ([#82](https://github.com/sneakypanel/Panel/issues/83), thanks [@xX1bumblebee1Xx](https://github.com/xX1bumblebee1Xx))
* Fixes a bug where new files could not be created through the panel for a server. ([#85](https://github.com/sneakypanel/Panel/issues/85), thanks [@xX1bumblebee1Xx](https://github.com/xX1bumblebee1Xx))
* Fixes the exception handler to properly display and log exceptions that might occur rather than leaving a vague error. ([#81](https://github.com/sneakypanel/Panel/issues/83))

### Changed
* Update Laravel to version `5.3` and update dependencies.

### Deprecated
* Requires sneakypanel Daemon `v0.2.*`

### Security
* Fixes listing of server variables for server. Previously a bug made it possible to view settings for all servers, even if the user didn't own that server. ([#69](https://github.com/sneakypanel/Panel/issues/69))
